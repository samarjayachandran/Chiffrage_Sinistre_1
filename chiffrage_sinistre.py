...
def render_product_selection(candidates: pd.DataFrame, key_prefix: str):
    ...
    with col_std:
        st.markdown(
            ...
            'margin-bottom:4px;"><b>Standard</b></div>',
    ...
    with col_lc:
        st.markdown(
            ...
            'margin-bottom:4px;"><b>Option bas carbone</b></div>',
            unsafe_allow_html=True,
        )
        if lc_df.empty:
...
def render_selection_panel(df: pd.DataFrame):
    """Single product-selection workflow. Two buttons let the user send the
    current selection to either Configuration 1 or Configuration 2."""

    KP = "shared"  # key prefix for all selection widgets
    ...
    if mode == "Recherche par catégorie":
        ...
        return  # baskets shown separately below
    ...
    # --- Live metrics for current selection ---
    ...
    # --- Build the entry dict (shared by both buttons) ---
    def _make_entry():
        entry = {
...
# =========================================================
# RENDER: BASKET  (smart cost handling — unchanged logic)
# =========================================================
...
# =========================================================
# RENDER: RUNNING TOTALS (always visible at the bottom)
# =========================================================
def render_running_totals():
    """Side-by-side live summary of both configurations."""
    ...
    with col1:
        st.markdown(
            ...
            <div style="padding:12px 16px;border:2px solid #1565c0;border-radius:8px;">
                <div style="font-weight:700;color:#1565c0;margin-bottom:6px;">
                    Configuration 1 — {len(b1)} ligne(s)
                </div>
                <div style="font-size:1.3em;">
                    🏭 <b>{total_co2_1:.2f}</b> kg CO₂
                </div>
                {"<div style='font-size:1.3em;'>💶 <b>" + f"{price1:.2f}" + "</b> €</div>" if price1 is not None else ("<div style='font-size:0.85em;color:#888;'>⚠️ Prix incomplets</div>" if b1 else "")}
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with col_sep:
        st.markdown(
            ...
            'height:100%;font-size:1.5em;color:#bbb;">vs</div>',
            unsafe_allow_html=True,
        )
    ...
    with col2:
        st.markdown(
            f"""
            <div style="padding:12px 16px;border:2px solid #6a1b9a;border-radius:8px;">
                <div style="font-weight:700;color:#6a1b9a;margin-bottom:6px;">
                    Configuration 2 — {len(b2)} ligne(s)
                </div>
                <div style="font-size:1.3em;">
                    🏭 <b>{total_co2_2:.2f}</b> kg CO₂
                </div>
                {"<div style='font-size:1.3em;'>💶 <b>" + f"{price2:.2f}" + "</b> €</div>" if price2 is not None else ("<div style='font-size:0.85em;color:#888;'>⚠️ Prix incomplets</div>" if b2 else "")}
            </div>
            """,
            unsafe_allow_html=True,
        )
...
# =========================================================
# RENDER: COMPARISON (reduction lives here exclusively)
# =========================================================
...
def main():
    ...
    tab_chiffrage, tab_baskets, tab_cmp = st.tabs([
        "🔧 Sélection produit",
        "📋 Paniers (Config 1 & 2)",
        "📊 Comparaison",
    ])
    ...
    with tab_chiffrage:
        ...
        # Running totals always visible at the bottom of the selection page
        st.divider()
